...
import {initDb, tableConfigRepo, tableDataRepo} from "../data/condition";
...
async function loadConfig() {
  let columns = await tableConfigRepo.findAll({name: {condition: '=', value: props.name}})
  if (!columns) {
    columns = []
  }
  console.log(columns)
...
async function loadData() {
  tableData.rows = await tableDataRepo.findAll({name: {condition: "=", value: props.name}});
}

function addRow() {
  excel.value.newRecord({name: props.name})
}
...
async function deleteSelected() {
  ...
  await tableDataRepo.deleteByIds(ids)
...
async function onUpdate(items) {
  await tableDataRepo.save(rowToUpdate);
  await loadData();
}
  ...
  <div style="width: 100%" id="excel">
    ...
    <vue-excel-editor ref="excel" v-model="tableData.rows" :filter-row="true" :new-if-bottom="true" autocomplete
      ...
      <vue-excel-column v-for="config in tableConfig.data" :field="config.field" :label="config.label"
                        :options="config.options?.split(',')" :invisible="config.visible ==='false'" :type="config.type"
                        :text-align="config.textAlign" :change="beforeUpdated"/>
    </vue-excel-editor>
  </div>
  <a-modal :open="tableData.configOpened" width="60%" @ok="ok"
           @cancel="ok">
    <ConfigTable :name="props.name"/>
  </a-modal>
